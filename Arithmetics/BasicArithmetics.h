...
float ChebyshevDistance(Point pt_1, Point pt_2) {
	if (pt_1.status_ != MATR || pt_2.status_ != MATR) {
		...
		if (dim_ == 1) {
			...
			float vec_x = std::abs(pt_1.x_ - pt_2.x_);
			float vec_y = std::abs(pt_1.y_ - pt_2.y_);
			return std::max(vec_x, vec_y);
		} else if (dim_ == 3) {
			float vec_x = std::abs(pt_1.x_ - pt_2.x_);
			float vec_y = std::abs(pt_1.y_ - pt_2.y_);
			float vec_z = std::abs(pt_1.z_ - pt_2.z_);
			return std::max(std::max(vec_x, vec_y), vec_z);
		} else {
			// IMPOSSIBLE
		}
	}
