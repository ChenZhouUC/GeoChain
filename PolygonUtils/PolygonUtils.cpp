...
bool segementIntersection(CvPoint p0, CvPoint p1, CvPoint p2, CvPoint p3,
													CvPoint* intersection) {
	CvPoint2D32f p;
	bool ok = segementIntersection(cvPoint32f(p0), cvPoint32f(p1), cvPoint32f(p2),
																 cvPoint32f(p3), &p);
	if (ok) *intersection = cvPoint((int)p.x, (int)p.y);
	return ok;
}

bool segementIntersection(CvPoint2D32f p0, CvPoint2D32f p1, CvPoint2D32f p2,
													CvPoint2D32f p3, CvPoint2D32f* intersection) {
	CvPoint2D32f s1, s2;
...
float Polygon::area() const { return computeArea(pt, n); }
...
void Polygon::pointsOrdered() {
	...
	for (int i = 0; i < n; i++) {
		...
		pc[i].p.y = pt[i].y;
		pc[i].angle =
				atan2f((float)(pt[i].y - center.y), (float)(pt[i].x - center.x));
	}
...
void intersectPolygon(const CvPoint* poly0, int n0, const CvPoint* poly1,
											int n1, Polygon& inter) {
	inter.clear();
	for (int i = 0; i < n0; i++) {
	...
	for (int i = 0; i < n1; i++) {
		if (pointInPolygon(poly1[i], poly0, n0)) inter.add(poly1[i]);
	}
...
void intersectPolygon(const Polygon& poly0, const Polygon& poly1,
											Polygon& inter) {
...
static int line_sect(const CvPoint* x0, const CvPoint* x1, const CvPoint* y0,
										 const CvPoint* y1, CvPoint* res) {
	CvPoint dx, dy, d;
	vsub(x1, x0, &dx);
	vsub(y1, y0, &dy);
	vsub(x0, y0, &d);
...
static void poly_edge_clip(const Polygon* sub, const CvPoint* x0,
													 const CvPoint* x1, int left, Polygon* res) {
	int i, side0, side1;
	CvPoint tmp;
	...
	for (i = 0; i < sub->n; i++) {
		...
		if (side0 + side1 == 0 && side0)
			...
			if (line_sect(x0, x1, v0, v1, &tmp)) res->add(tmp);
		if (i == sub->n - 1) break;
...
void intersectPolygonSHPC(const Polygon* sub, const Polygon* clip,
													Polygon* res) {
	int i;
	Polygon P1, P2;
	Polygon* p1 = &P1;
